...
#[no_mangle]
extern "C" fn callback(target: *mut mpsc::Sender<i32>, a: i32) {
    println!("I'm called from C with value {0}", a);
    unsafe {
        (*target).send(a).unwrap();
    }
}
...
extern {
    fn register_callback(target: *mut mpsc::Sender<i32>,
                        cb: extern fn(*mut mpsc::Sender<i32>, i32)) -> extern "C" fn(i32);
}
#[link(name = "c_main")]
extern {
...
fn ffi_loop(tx:mpsc::Sender<i32>) -> ! {
    ...
    let mut rust_object =  Box::new(tx);
